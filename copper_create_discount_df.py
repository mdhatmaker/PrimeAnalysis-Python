...
def get_quandl_cal_symbols(cal, instrument="HG"):
    ...
    m2 = cal[3]
    y2 = "20" + cal[4:6]
    symbol1 = "CME/" + instrument + m1 + y1
    symbol2 = "CME/" + instrument + m2 + y2
    return (m1, y1, m2, y2, symbol1, symbol2)
...
def get_calendar_data(cal, instrument="HG"):
    (m1, y1, m2, y2, symbol1, symbol2) = get_quandl_cal_symbols(cal)
    print cal, m1, y1, m2, y2, symbol1, symbol2
    get_month_number(m1)
    dt2 = datetime(y1, m1, 31)
    dt1 = d2.replace(day=1)
    ...
    dts = ""
    dte = ""
    if (m1 == "H" and (m2 == "J" or m2 == "K")):
        dts = y_prev + "-9-1"
        dte = y2 + "-3-31"
    elif (m1 == "K" and (m2 == "M" or m2 == "N")):
        dts = y_prev + "-12-1"
        dte = y2 + "-5-31"
    elif (m1 == "N" and (m2 == "Q" or m2 == "U")):
        dts = y1 + "-3-1"
        dte = y2 + "-7-31"
    elif (m1 == "U" and (m2 == "V" or m2 == "X" or m2 == "Z")):
        dts = y1 + "-5-1"
        dte = y2 + "-9-30"
    elif (m1 == "Z" and (m2 == "F" or m2 == "G" or m2 == "H")):
        dts = y1 + "-7-1"
        dte = y2 + "-12-31"
    else:
        raise ValueError
